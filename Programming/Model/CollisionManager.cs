...
namespace Programming.Model
{
    static class CollisionManager
    {
        ...
        {
            if((Math.Abs(rectangular1.Center.X - rectangular2.Center.X) < Math.Abs(rectangular1.Width - rectangular2.Width) / 2) &&
               (Math.Abs(rectangular1.Center.Y - rectangular2.Center.Y) < Math.Abs(rectangular1.Length - rectangular2.Length) / 2))
            {
                return true;
            }
            else
